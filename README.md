...
1. Build the application image, with the Git commit and the jj change ID of the code:

   ```sh
   CDPP_COMMIT=$(git rev-parse HEAD) \
   CDPP_CHANGE=$(jj log -r @- --no-graph -T change_id) \
...
3. Deploy, with the Git commit and the jj change ID of the code:

   ```sh
   fly deploy --ha=false \
     --build-arg CDPP_COMMIT=$(git rev-parse HEAD) \
     --build-arg CDPP_CHANGE=$(jj log -r @- --no-graph -T change_id)
   ```
...
| `CDPP_CHANGE` | Not set | jj change ID of the code. A downloaded photograph contains it as `cdp:applicationChange`. The image sets it from the build argument `CDPP_CHANGE`. |
...
| `cdp:applicationChange` | The jj change ID of that commit, if `CDPP_CHANGE` is set. A change ID stays the same when the description of the commit changes, but Git does not show it. |
