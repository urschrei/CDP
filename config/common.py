# no debugging by default - this is overriden in run.py for local dev
DEBUG = False

# currently the same for all environments
SQLALCHEMY_DATABASE_URI = 'mysql+mysqldb://glyph:glyph@127.0.0.1/glyph?charset=utf8&use_unicode=0'
# SQLALCHEMY_DATABASE_URI = 'postgresql://glyph:glyph@localhost:5432/glyph'

# os.urandom(128).encode('hex') should do the trick
SECRET_KEY = ''
