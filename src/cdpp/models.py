...
class Tablet(Entity):
    ...
    sent_from: Mapped[Correspondent | None] = relationship()
...
# Rules for values that the schema stores twice. Each rule has a name, a table,
# the columns that a change can make inconsistent, the statements that the rule
# checks, the condition of a contradiction in the new row, and a message. The
# migration that adds the rules creates the same triggers.
CONSISTENCY_RULES = (
    (
        "tablet_sub_period",
        "tablet",
...
def consistency_triggers() -> list[tuple[str, str]]:
    """Return the table and the CREATE TRIGGER statement of each rule."""
    triggers = []
    for name, table, columns, statements, condition, message in CONSISTENCY_RULES:
...
def check_consistency() -> None:
    """Make the tables of CONSISTENCY_RULES refuse contradictions.

    The triggers do not check the rows that exist when they are made.
    """
    for table_name, statement in consistency_triggers():
        event.listen(
            db.metadata.tables[table_name],
            "after_create",
